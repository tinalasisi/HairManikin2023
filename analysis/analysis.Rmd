...
# filter originally imported df for radiation on and dry heat loss only
df_dry_on <- df_wetdry %>% 
...
# take df created above for temperature corrected (30C) dry heat loss with no solar radiation and subtract solar influx (therewith adding the heat loss attributed to solar radiation)
...
# create final df by binding the complete temperature corrected (30C) df with the wet data for radiation on, radiation off, and solar influx
df_wetdry_final <- bind_rows(df_dry30, df_influx[df_influx$wet_dry == "wet",]) %>% 
  distinct()

df_wetdry_final

write_csv(df_wetdry_final, F("data/df_wetdry_final.csv"))

# here we create the same table but in long pivoted format for some of the analyses.
...
write_csv(df_wetdry_final, F("data/df_long_wetdry_final.csv"))
...
# For visualization purposes, we summarize the df created above to provide mean, min, and max for each condition
...
write_csv(df_wetdry_final, F("data/sum_manikin_df.csv"))
...
$$Combined \ heat\ loss (W/m^2)_{(30C\ wet+dry +  radiation)}  = Dry\ heat\ loss (W/m^2)_{(30C\ no\ radiation)} +  Wet\ heat\ loss (W/m^2)_{(30C\ + radiation)} $$
$$Combined \ influx (W/m^2)_{(30C\ wet+dry)}  = Combined \ heat\ loss (W/m^2)_{(30C\ wet+dry + no \ radiation)}  - Combined \ heat\ loss (W/m^2)_{(30C\ wet+dry +  radiation)}  $$

```{r df-wetdry-sum}

# takiing the summary df created above, we create combined wet and dry values for each condition
...
write_csv(df_sum_wetdry, F("data/df_sum_wetdry.csv"))
# 
# sum_manikin_df %>% 
#   select(-n) %>%
#   # select(-c(min, max, n)) %>% 
...
#          min.wetdry_on = min_dry_off + min_wet_on,
#          min.wetdry_influx = min.wetdry_off - min.wetdry_on) %>% 
#   select(c(wig, wind, mean_wet_off, mean.wetdry_off, mean_wet_on, mean.wetdry_on))
  

```

```{r plt-wetdry-sum, fig.height=3, fig.width=7}
...
# unscaled 
plt_sum_dry <- plot_summs(model_dry_off, model_dry_on, model_dry,  model.names = c("Radiation Off", "Radiation On", "Solar Influx")) + xlim(-200, 200)
...
# unscaled 
plt_sum_wet <- plot_summs(model_wet_off, model_wet_on, model_wet,  model.names = c("Radiation Off", "Radiation On", "Solar Influx")) + xlim(-200, 200)
...
```{r plt-lm-combo-sep, echo=FALSE, include=FALSE}

plt_sum_dry + plt_sum_wet + plot_annotation(tag_levels = "A") +  plot_layout(ncol = 1, guides = "collect") & theme(legend.position = "bottom") 
