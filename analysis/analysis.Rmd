...
df_wetdry <- read_csv(F("data/current/ManikinData_WetDry.csv"), 
  ...
  mutate(wig = factor(wig, levels = c("Nude", 
        "LowCurv", "MidCurv", "HighCurv"), labels = c("Nude", 
        "Straight", "Slightly\nCurled", "Tightly\nCurled"))) %>% 
  rename(heat_loss = heatloss)

df_wetdry_plots <- df_wetdry %>% 
  unite('data_points', c('wet_dry','trial'), remove=FALSE)
...
ggplot(df_wetdry_plots, aes(x = wig, y = heat_loss, fill = data_points)) + 
  ...
  scale_fill_manual(values = colourslist, breaks = c('dry_1', 'wet_1'), labels = c('Dry Hair', 'Wet Hair')) + 
...
change_winds <- function(df, windCol, wigCol) {
  ...
  # df[windCol] <- df[windCol] + (20)*(ifelse(df[wigCol] == "Nude", -1, 1))
...
# SCATTER PLOT
ggplot(df_wetdry_plots, aes(x = wind, y = heat_loss, shape = wig, fill = wet_dry)) +
  geom_point(size = 2) +
...
ggplot(df_net_influx_plots, aes(x = wig, y = net_influx, fill = wet_dry)) + 
...
ggplot(df_adjusted_solar_plots, aes(x = wig, y = heat_loss, fill = wet_dry)) + 
  geom_bar(stat = "identity", position = "dodge", color = "black") + 
  facet_grid(. ~ wind, labeller = labeller(wind = as_labeller(wind_names, label_parsed)))+ 
  scale_fill_manual(values = c(dryHairColor, wetHairColor), labels = c("Dry Heat Loss", "Wet Heat Loss")) + 
  labs(title = 'Heat Loss with Solar Radiation', x = "", y = expression(paste("Heat Loss ( ", frac("W","m"^"2"), ' )')), fill = '') + 
scale_color_discrete(guide = "none") +  theme_bw()


ggplot(df_adjusted_solar_plots, aes(x = wind, y = heat_loss, shape = wig, fill = wet_dry)) +
  geom_point(size = 3) +
  geom_line() +
  scale_fill_manual(values = c(dryHairColor, wetHairColor), labels = c("Dry Heat Loss", "Wet Heat Loss")) +
  scale_shape_manual(values = c(21,22,23,24)) + 
  ...
  scale_x_continuous(limits = c(0,2.7), breaks = c(0, 0.5, 1, 1.5, 2, 2.5)) +
  guides(fill = guide_legend(override.aes=list(shape=21))) +
  labs(title = 'Heat Loss with Solar Radiation', x = expression(paste("Wind Speed ( ", frac("m", "s"), ' )')), y = expression(paste("Heat Loss ( ", frac("W","m"^"2"), ' )'))) +
theme_bw() + theme( panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
...
ggplot(df_adjusted_solar_plots, aes(x = wind, y = heat_loss, shape = wig, fill = wet_dry)) +
  geom_point(size = 3) +
  geom_line() +
  facet_grid(wet_dry ~ ., labeller = as_labeller(c(`dry_heat_loss` = 'Dry Hair', `wet_heat_loss` = 'Wet Hair')))+ 
  scale_fill_manual(values = c(dryHairColor, wetHairColor), labels = c("Dry Heat Loss", "Wet Heat Loss")) +
  scale_shape_manual(values = c(21,22,23,24)) + 
...
ggplot(df_sweat_requirements, aes(x = wind, y = sweat_zero, shape = wig, fill = wig)) +
  geom_point(size = 4) +
  scale_shape_manual(values = c(21,22,23,24)) + 
  scale_color_manual(values=c("blue", "red", "green", "pink")) +
  ...
  scale_x_continuous(limits = c(0,2.7), breaks = c(0, 0.5, 1, 1.5, 2, 2.5)) +
  guides(fill = guide_legend(override.aes=list(shape=21))) +
  labs(title = "Sweat Required to Achieve Zero Heat Gain", x = expression(paste("Wind Speed ( ", frac("m", "s"), ' )')), y = expression(paste("Sweat ( ", frac("g","m"^"2"*"h"), ' )'))) +
theme_bw() + theme( panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))

ggplot(df_sweat_requirements, aes(x = wind, y = sweat_max, shape = wig, fill = wig)) +
  geom_point(size = 4) +
  geom_line() +
  scale_shape_manual(values = c(21,22,23,24)) + 
  ...
  scale_x_continuous(limits = c(0,2.7), breaks = c(0, 0.5, 1, 1.5, 2, 2.5)) +
  guides(fill = guide_legend(override.aes=list(shape=21))) +
  labs(title = "Maximum Possible Sweat Evaporation", x = expression(paste("Wind Speed ( ", frac("m", "s"), ' )')), y = expression(paste("Sweat ( ", frac("g","m"^"2"*"h"), ' )'))) +
theme_bw() + theme( panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
```
