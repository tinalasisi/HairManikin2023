...
sweat_combo_plot <- ggplot(df_sweat_requirements_long, aes(x = wind, y = value, shape = wig, fill = wig)) +
  geom_point(size = 3) +
  geom_line() +
  facet_grid(.~ sweat , labeller = as_labeller(c(`sweat_max` = 'A. Max sweat', `sweat_zero` = 'B. Zero heat gain')))+   scale_shape_manual(values = c(21,22,23,24)) + 
  scale_fill_manual(values=c("red","blue","pink","orange")) +
...
manuscript_plt6 <- (manuscript_plt_dry_lm)+ (manuscript_plt_wet_lm + theme(axis.text.y = element_blank())) + plot_layout(guides = "collect") & theme(legend.title = element_blank())
